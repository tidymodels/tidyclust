...
```{r}
kmeans_summary$within_sse
kmeans_summary$tot_sse
...
```{r, echo = FALSE}
pens <- penguins %>%  select(bill_length_mm, bill_depth_mm) %>% drop_na()

init <- sample(1:nrow(pens), 3)
...
closest_center <- Rfast::dista(as.matrix(pens), as.matrix(pens[init,])) %>% apply(1, which.min)

pens %>%
  ggplot() +
  geom_point(data = pens, 
               aes(x = bill_length_mm, y = bill_depth_mm,
...
pens %>%
  ...
  geom_mark_ellipse(aes(x = bill_length_mm,
                        y = bill_depth_mm,
                        color = as.factor(thing$cluster)),
                    expand = unit(1, "mm"))  +
  geom_point(data = centers, 
