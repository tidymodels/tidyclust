...
All tidyclust metrics accept a `dist_fun` argument that controls how distances
are computed. The `{philentropy}` package, which tidyclust uses internally,
supports many distance and similarity measures. To see all available methods:

```{r}
philentropy::getDistMethods()
```

There are two different `dist_fun` interfaces depending on which metric you are
using.

### Silhouette metrics

`silhouette_avg()` and `silhouette()` expect a single-argument function that
takes a data frame and returns a `dist` object — the same interface as
`philentropy::distance` (the default):
...
`sse_within_total()`, `sse_total()`, and `sse_ratio()` expect a two-argument
function `function(x, y)` that computes distances between two matrices (the
cluster centroids and the observations). The default is
`philentropy::dist_many_many` with Euclidean distance:
