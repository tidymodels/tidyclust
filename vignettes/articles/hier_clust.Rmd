...
in the hierarchical output.  This can be accomplished with `reconcile_clusterings()`.

This function expects as input two vectors of cluster labels.  The first is the
labels that will be matches, and the second is the labels that will be recoded
to the first.
...
```{r}
reconcile_clusterings(primary_cluster_assignment = hc_preds$.pred_cluster, 
