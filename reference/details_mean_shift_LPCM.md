...
formula method via
[`fit()`](https://tidyclust.tidymodels.org/reference/fit.md), tidyclust
will convert factor columns to indicators.

[`LPCM::ms()`](https://rdrr.io/pkg/LPCM/man/ms.html) scales each
variable internally to the unit range before applying the Gaussian
kernel, so the `bandwidth` value lives on the scaled scale rather than
the raw data scale. Bandwidths between roughly `0.05` and `1` are
typical; smaller values find more clusters and larger values merge them.
...
training mode by Euclidean distance.
...
- Einbeck, J., Evers, L., & Hinchliff, K. (2010). Data compression and
  ...
  Seidel, & A. Ultsch (Eds.), Advances in Data Analysis, Data Handling
  and Business Intelligence (pp. 701–712). Springer.
