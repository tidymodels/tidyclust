...
<!-- badges: start -->
[![CRAN status](https://www.r-pkg.org/badges/version/tidyclust)](https://CRAN.R-project.org/package=tidyclust)
...
The goal of tidyclust is to provide a tidy, unified interface to clustering models. The package is closely modeled after the [parsnip](https://parsnip.tidymodels.org/) package.
...
make_circles <- function(n) {
  ...
  normalize(data.frame(x, y), "circles")
}
...
make_halves <- function(n) {
  ...
  normalize(data.frame(x, y), "halves")
}
...
make_uniform <- function(n) {
  ...
  normalize(data.frame(x, y), "uniform")
}
...
make_blobs <- function(n) {
  ...
  normalize(data.frame(x, y), "blobs")
}
...
colors <- c("#E49E68", "#6899E4", "#E068E4", rep("#BBBBBB", 100))

expand_grid(
  models = list(
    k_means(num_clusters = 3),
    hier_clust(num_clusters = 3),
    db_clust(radius = 0.25, min_points = 5),
    gm_clust(num_clusters = 3),
    mean_shift(bandwidth = 0.1)
  ),
  datasets = list(
    circle_data,
    halves_data,
  ...
  pmap_dfr(\(models, datasets) augment_model(models, datasets)) |>
